...
or given an optional client script (script family: Tabs, Menu, ContextMenu, Menubar, Tooltip, PreviewCard,
...
| `script-adapter` | Behavior from an optional client script in `public/shadcn/` (Tabs, DropdownMenu, ContextMenu, Menubar, Tooltip, HoverCard, [ADR 0025](./adr/0025-ship-optional-client-scripts-for-behavior-the-browser-does-not-provide.md)) |
...
| Interactive behavior (keyboard, focus, ARIA, no scripts) and open-state screenshots against upstream | `tests/visual/modals.spec.ts` (Dialog, AlertDialog, Sheet), `tests/visual/disclosure.spec.ts` (Accordion, Collapsible), `tests/visual/controls.spec.ts` (form controls), `tests/visual/popover.spec.ts`, `tests/visual/select.spec.ts`, `tests/visual/tabs.spec.ts`, `tests/visual/menu.spec.ts` and `tests/visual/hover.spec.ts` (step-by-step behavior against Base UI) | `bun run test:visual`, CI `visual` |
