...
or given an optional client script (script family: Tabs, Menu, ContextMenu, Menubar, Tooltip, PreviewCard, Slider, Combobox,
...
| `script-adapter` | Behavior from an optional client script in `public/shadcn/`, through a `script` family (Tabs, DropdownMenu, ContextMenu, Menubar, Tooltip, HoverCard, Slider, Combobox) or a `script` component adapter that moves an upstream event handler into a script (InputGroup: focusing the input from an addon), [ADR 0025](./adr/0025-ship-optional-client-scripts-for-behavior-the-browser-does-not-provide.md)) |
