...
ui.shadcn.com registries (every Base UI style: base-nova, base-vega, …)
  ...
  → catalog/manifest   → cli/generated/catalog.json, compatibility.json, docs/compatibility.md
...
or given an optional client script (script family: Tabs, Menu, ContextMenu, Menubar, Tooltip, PreviewCard, Slider, Combobox, NavigationMenu, Avatar, ScrollArea, Collapsible, Drawer, Toast,
...
| `script-adapter` | Behavior from an optional client script in `public/shadcn/` ([ADR 0025](./adr/0025-ship-optional-client-scripts-for-behavior-the-browser-does-not-provide.md)), through a `script` family (Tabs, DropdownMenu, ContextMenu, Menubar, Tooltip, HoverCard, Slider, Combobox, NavigationMenu, Avatar, ScrollArea; Drawer on the native `<dialog>` with swiping from its script; Toast with `toast.add()` exported by its script, [ADR 0027](./adr/0027-add-toasts-from-the-toast-script-in-the-browser-and-from-toaster-props-on-the-server.md); Collapsible, which stays native `<details>` when its trigger is its first child, [ADR 0026](./adr/0026-toggle-a-collapsible-with-a-script-when-its-trigger-is-not-the-first-child.md)) or a `script` component adapter that replaces upstream React state and handlers with props on the server and its script in the browser (InputGroup: focusing the input from an addon; Sidebar: toggling, the cookie, the shortcut and the mobile sheet) |
| `custom-adapter` | Blocking reasons all resolved by an adapter in `generator/src/adapters/components/` without browser behavior (Direction: `DirectionProvider` on a Hono context) |
