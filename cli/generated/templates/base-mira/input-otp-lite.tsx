      ...
      >
        {Array.from({ length: maxLength }, (_, index) => (
          <div
            ...
            class="relative flex size-7 items-center justify-center border-y border-r border-input bg-input/20 text-xs/relaxed transition-all outline-none first:rounded-l-md first:border-l last:rounded-r-md group-has-aria-invalid/input-otp:border-destructive group-has-focus-visible/input-otp:border-ring dark:bg-input/30"
          />
        ))}
      </div>
      {/*
      ...
      */}
      <div class="absolute inset-0 overflow-hidden rounded-md">
        <input
          ...
          minlength={maxLength}
          class={cn(
            "absolute top-0 start-0 h-full w-[calc((var(--input-otp-length)+1)*--spacing(7))] border-0 bg-transparent pb-px pl-[calc((--spacing(7)-1ch)/2)] text-xs/relaxed tracking-[calc(--spacing(7)-1ch)] text-foreground tabular-nums outline-none disabled:cursor-not-allowed rtl:[unicode-bidi:bidi-override] rtl:translate-x-[calc(--spacing(7)-1ch)]",
            className
          )}
          style={style}
          {...props}
