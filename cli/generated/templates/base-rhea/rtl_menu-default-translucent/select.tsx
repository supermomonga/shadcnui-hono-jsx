...
// upstream: shadcn/ui base-rhea/select (https://ui.shadcn.com/r/styles/base-rhea/select.json)
// upstream-revision: sha256:c763d9540abc384c716330bcb0e24787497690fec7ae2fe9383810424ef8da30; shadcn-ui/ui@98a1fe67b439324ddc857f47fbdce056600a4329
// mode: generated
...
const SELECT_PICKER_CLASSES =
  "[appearance:base-select] [&::picker-icon]:hidden [&::picker(select)]:[appearance:base-select] [&::picker(select)]:border-0 [&::picker(select)]:p-0 [&::picker(select)]:[margin:var(--select-picker-margin)] [&::picker(select)]:[position-area:var(--select-picker-area)] [&::picker(select)]:[position-try-fallbacks:flip-block] [&::picker(select)]:transition-[display,overlay] [&::picker(select)]:transition-discrete [&::picker(select)]:max-h-(--available-height) [&::picker(select)]:w-(--anchor-width) [&::picker(select)]:min-w-36 [&::picker(select)]:origin-(--transform-origin) [&::picker(select)]:overflow-x-hidden [&::picker(select)]:overflow-y-auto [&::picker(select)]:rounded-2xl [&::picker(select)]:text-popover-foreground [&::picker(select)]:shadow-lg [&::picker(select)]:ring-1 [&::picker(select)]:ring-foreground/5 [&::picker(select)]:duration-100 data-[side=bottom]:[&::picker(select)]:slide-in-from-top-2 data-[side=inline-end]:[&::picker(select)]:slide-in-from-start-2 data-[side=inline-start]:[&::picker(select)]:slide-in-from-end-2 data-[side=left]:[&::picker(select)]:slide-in-from-right-2 data-[side=right]:[&::picker(select)]:slide-in-from-left-2 data-[side=top]:[&::picker(select)]:slide-in-from-bottom-2 dark:[&::picker(select)]:ring-foreground/10 open:[&::picker(select)]:animate-in open:[&::picker(select)]:fade-in-0 open:[&::picker(select)]:zoom-in-95 not-open:[&::picker(select)]:animate-out not-open:[&::picker(select)]:fade-out-0 not-open:[&::picker(select)]:zoom-out-95 [&::picker(select)]:animate-none! [&::picker(select)]:bg-popover/70 [&::picker(select)]:backdrop-blur-2xl [&::picker(select)]:backdrop-saturate-150 **:data-[slot$=-item]:focus:bg-foreground/10 **:data-[slot$=-item]:focus:bg-foreground/10 **:data-[slot$=-separator]:bg-foreground/5 **:data-[slot$=-trigger]:focus:bg-foreground/10 **:data-[slot$=-trigger]:aria-expanded:bg-foreground/10! **:data-[variant=destructive]:focus:bg-foreground/10! **:data-[variant=destructive]:text-accent-foreground! **:data-[variant=destructive]:**:text-accent-foreground!"
...
function SelectTrigger({
  ...
  return (
    <SelectTriggerElement
      ...
      data-size={size}
      class={cn(
        "flex w-fit items-center justify-between gap-1.5 rounded-2xl border border-transparent bg-input/50 px-3 py-2 text-sm whitespace-nowrap transition-[color,box-shadow] duration-200 outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 has-[option[data-placeholder]:checked]:text-muted-foreground data-[size=default]:h-8 data-[size=sm]:h-7 **:data-[slot=select-value]:line-clamp-1 **:data-[slot=select-value]:flex **:data-[slot=select-value]:items-center **:data-[slot=select-value]:gap-1.5 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
...
function SelectItem({ class: className, children, ...props }: SelectItemProps) {
  return (
    <SelectItemElement
      ...
      class={cn(
        "relative flex min-h-7 w-full cursor-default items-center gap-2 rounded-xl py-1.5 pe-8 ps-2 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2 [&::checkmark]:hidden disabled:text-inherit",
        className
      )}
      {...props}
    >
...
function SelectSeparator({ class: className, ...props }: ComponentProps<"hr">) {
  return (
    <hr
      ...
      class={cn(
        "pointer-events-none -mx-1 my-1 h-px bg-border border-0",
        className
      )}
      {...props}
    />
