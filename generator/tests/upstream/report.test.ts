...
describe("renderSyncReport", () => {
  test("summarizes changes, highlights generated components and shows diffs", () => {
    ...
    expect(report).toContain("<summary>base-nova/button (generated)</summary>")
  ...
  test("puts license changes first with a caution to block merging", () => {
    ...
    expect(report.indexOf("LICENSE.md (shadcn-ui/ui)")).toBeLessThan(
      report.indexOf("<summary>base-nova/kbd")
    )
  ...
  test("names components once across styles and shows diffs of the default style", () => {
    const change = (style: string, name: string) => ({
      style,
      name,
      before: item(name, `${style} a\n`),
      after: item(name, `${style} b\n`),
    })
    const report = renderSyncReport({
      style: "base-nova",
      styles: ["base-nova", "base-lyra"],
      generated: ["button"],
      classifications: [],
      result: {
        ...empty,
        changed: [
          change("base-nova", "button"),
          change("base-lyra", "button"),
          change("base-lyra", "card"),
        ],
      },
    })
    expect(report).toContain("- Changed: `button`, `card` (base-lyra)")
    expect(report).toContain("<summary>base-nova/button (generated)</summary>")
    expect(report).not.toContain("<summary>base-lyra/button")
    expect(report).toContain("<summary>base-lyra/card</summary>")
  })
})
